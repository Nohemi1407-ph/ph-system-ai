...
export async function generarAnuncio(jobId, { apiKey, prompt, ratio, resolucion, duracion, imagenes = [], videos = [] }) {
  ...
  try {
    ...
    const preVuelo = await verificarSaldo({
      ...
      etiqueta: `un anuncio de ${duracion}s`,
    });
    ...
    actualizarJob(jobId, {
      ...
      modelo: 'Seedance 2.5',
      mensaje: `Generando ${duracion}s con Seedance 2.5...`,
    });

    const r = await ejecutar({
      modeloId: 'seedance-2-5',
      apiKey,
      params: {
        prompt,
        image_urls: imagenes,
    ...
    console.error(`[Marketing] Seedance 2.5 fallo (${err.codigo}):`, err.message);
