 ...
 *
 *   CALIDAD   1080p usa veo3 (el modelo bueno); 720p usa veo3_fast.
 *
...
const COSTO_CLIP_RAPIDO = 0.4;
const COSTO_CLIP_CALIDAD = 1.6;
...
export function modeloDe(resolucion) {
  return resolucion === '1080p' ? 'veo3' : 'veo3_fast';
}

export function costoEstimado({ duracion, resolucion }) {
  const porClip = resolucion === '1080p' ? COSTO_CLIP_CALIDAD : COSTO_CLIP_RAPIDO;
...
export async function generarAnuncio(jobId, { apiKey, prompt, ratio, resolucion, duracion, imagenes = [] }) {
  ...
  const gasto = contadorDeGasto({
    estimadoPorClipUsd: resolucion === '1080p' ? COSTO_CLIP_CALIDAD : COSTO_CLIP_RAPIDO,
  });
