 ...
 */
function construirPrompt({ prompt, referencias, ratio, continuacion = false, identidadProducto = '' }) {
  const mapa = {};
  ...
  return JSON.stringify({
    // Lo PRIMERO y con el nombre mas explicito posible: lo que escribio el
    // vendedor son instrucciones, no un dato mas. Antes iba como "ad_script",
    // un campo entre cinco, al mismo nivel que "avoid" o "framing", y el modelo
    // lo trataba como un parametro cualquiera.
    ...
    // La identidad exacta del producto, leida de su foto. Sin esto el modelo
    // rellena los huecos inventando etiqueta, color y forma: es la causa
    // numero uno de que el producto salga "parecido" pero no sea el mismo.
    ...
    ...(continuacion
      ...
      : {}),
    // Sugerencia, no orden: solo aplica si el vendedor no dijo otra cosa.
...
export async function generarAnuncio(jobId, { apiKey, prompt, ratio, resolucion, duracion, imagenes = [], videos = [] }) {
  ...
  try {
    ...
    const r = await ejecutar({
      ...
      apiKey,
      params: {
        prompt: construirPrompt({ prompt, referencias: imagenes, ratio, identidadProducto }),
        image_urls: imagenes,
        video_urls: videos,
        duration: duracion,
    ...
    const base = await generarVideo(
      {
        ...
        prompt: construirPrompt({ prompt, referencias: imagenes, ratio: nativo, identidadProducto }),
        imagenes,
        aspect_ratio: nativo,
      },
      apiKey
