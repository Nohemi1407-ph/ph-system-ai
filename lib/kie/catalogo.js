...
/**
 * Input de las variantes 2.0 (Fast y Mini). Mismo esquema entre ellas, con
 * topes mas bajos que la 2.5: 9 imagenes, 3 videos, 4-15 s y hasta 720p.
 */
function construirSeedance2(p) {
  return {
    prompt: p.prompt,
    reference_image_urls: (p.image_urls || [p.image_url]).filter(Boolean).slice(0, 9),
    ...(p.video_urls?.length ? { reference_video_urls: p.video_urls.slice(0, 3) } : {}),
    duration: Math.max(4, Math.min(15, Number(p.duration) || 5)),
    // Estos modelos no llegan a 1080p: pedirlo devolveria un error del
    // proveedor, asi que se baja a lo maximo que dan.
...
export const MODELOS = [
  ...
  {
    id: 'seedance-2-fast',
    modelo: 'bytedance/seedance-2-fast',
    name: 'Seedance 2.0 Fast · ByteDance (barato)',
    tarea: 'i2v',
    endpoint: 'market',
    maxImagenes: 9,
    inputs: {
      prompt: campoPrompt('Describe el video.'),
      aspect_ratio: campoRatio(['adaptive', '1:1', '4:3', '3:4', '16:9', '9:16', '21:9'], 'adaptive'),
      resolution: campoResolucion(['480p', '720p'], '720p'),
      duration: {
        name: 'duration',
        title: 'Duration',
        type: 'number',
        enum: Array.from({ length: 12 }, (_, i) => i + 4),
        default: 5,
      },
    },
    construir: (p) => construirSeedance2(p),
  },
  {
    id: 'seedance-2-mini',
    modelo: 'bytedance/seedance-2-mini',
    name: 'Seedance 2.0 Mini · ByteDance (el mas barato)',
    tarea: 'i2v',
    endpoint: 'market',
    maxImagenes: 9,
    inputs: {
      prompt: campoPrompt('Describe el video.'),
      aspect_ratio: campoRatio(['adaptive', '1:1', '4:3', '3:4', '16:9', '9:16', '21:9'], 'adaptive'),
      resolution: campoResolucion(['480p', '720p'], '480p'),
      duration: {
        name: 'duration',
        title: 'Duration',
        type: 'number',
