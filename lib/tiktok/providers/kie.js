...
async function esperar(url, apiKey, { intentos = 300, intervalo = 3000, onTick, taskId = null } = {}) {
  ...
  while (Date.now() < limite) {
    ...
    if (ESTADOS_FALLO.includes(estado)) {
      ...
      // Solo el motivo, no el volcado entero de la respuesta. El payload
      // completo acababa guardado en el job y devuelto por la API en cada
      // sondeo: respuestas de decenas de KB por un error de una linea. Y como
      // el almacen se reescribe entero en cada actualizacion, ese peso se
      // pagaba muchas veces. El volcado va al log del servidor, que es su sitio.
      console.error(`[Kie] Tarea ${taskId} fallida:`, JSON.stringify(data).slice(0, 2000));
      throw clasificar(new Error(`Kie falló: ${detalle}`), { taskId });
    }
  }
...
// ─── Mercado: cualquier modelo, cualquier input ──────────────────────────────

/**
 * Crea una tarea en el mercado de Kie y espera el resultado.
 *
 * Es el camino generico: todos los modelos del mercado (imagen, video, lip
 * sync, video a video) usan el mismo protocolo y solo cambian el `model` y el
 * `input`. Quien llama decide esos dos; aqui no se asume nada de su forma.
 */
export async function crearTarea({ modelo, input }, apiKey, opts) {
  const creado = await pedir(
    `${API}/api/v1/jobs/createTask`,
    { method: 'POST', body: JSON.stringify({ model: modelo, input }) },
    apiKey
  );

  const taskId = creado.data?.taskId;
  if (!taskId) throw new ErrorVideo(CODIGOS.PROVIDER_ERROR, `Kie no devolvió taskId para ${modelo}.`);
  opts?.onRequestId?.(taskId);

  const r = await esperar(`${API}/api/v1/jobs/recordInfo?taskId=${taskId}`, apiKey, {
    // Los modelos de video del mercado tardan minutos; los de imagen, segundos.
    // El sondeo es adaptativo, asi que un presupuesto amplio no penaliza.
    intentos: 300,
    intervalo: 3000,
    ...opts,
    taskId,
 ...
 */
// El modo de referencias solo lo admiten estos modelos. Con cualquier otro,
// Kie responde HTTP 200 con el error dentro y la tarea nunca se crea.
const MODELOS_CON_REFERENCIAS = ['veo3_fast', 'veo3_lite'];

export async function generarVideo({ modelo = 'veo3_fast', modo, prompt, imagenes = [], aspect_ratio = '9:16' }, apiKey, opts) {
  if (modo === 'REFERENCE_2_VIDEO' && !MODELOS_CON_REFERENCIAS.includes(modelo)) {
    // Se corrige en vez de fallar: pedir referencias es la intencion, y el
    // modelo es un detalle. Antes esto llegaba al proveedor y volvia como un
    // error opaco a mitad de un trabajo ya empezado.
    console.warn(
      `[Kie] ${modelo} no admite referencias; se usa veo3_fast. Modelos válidos: ${MODELOS_CON_REFERENCIAS.join(', ')}.`
    );
    modelo = 'veo3_fast';
  }
