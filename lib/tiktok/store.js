...
let respaldo = { jobs: {}, feedback: {}, secuencia: 0 };

function leer() {
  try {
    if (fs.existsSync(ARCHIVO)) {
      const data = JSON.parse(fs.readFileSync(ARCHIVO, 'utf8'));
      respaldo = { jobs: {}, feedback: {}, secuencia: 0, ...data };
      return respaldo;
    }
  } catch (e) {
...
/**
 * Cuantos jobs se conservan.
 *
 * El almacen se reescribe ENTERO en cada actualizacion, y un solo trabajo hace
 * entre 6 y 17 actualizaciones. Sin poda, el archivo crece para siempre y cada
 * actualizacion de progreso se vuelve mas lenta que la anterior. Se guardan los
 * mas recientes, que es lo unico que la interfaz llega a mostrar.
 */
const MAX_JOBS = Number(process.env.PH_MAX_JOBS) || 100;

function podar(data) {
  const ids = Object.keys(data.jobs);
  if (ids.length <= MAX_JOBS) return data;

  // Se ordena por `secuencia`, un contador que solo sube. Con `creado` solo, dos
  // jobs del mismo milisegundo quedaban en orden arbitrario y la poda podia
  // llevarse uno recien creado.
  const vivos = ids
    .sort((a, b) => (data.jobs[b].secuencia || 0) - (data.jobs[a].secuencia || 0))
    .slice(0, MAX_JOBS);

  const jobs = {};
  for (const id of vivos) jobs[id] = data.jobs[id];
  data.jobs = jobs;

  // El feedback apunta a jobs por id: se limpia lo que ya no existe.
  for (const [clave, f] of Object.entries(data.feedback || {})) {
    if (!jobs[f.jobId]) delete data.feedback[clave];
  }
  return data;
}
...
export function crearJob(job) {
  const data = leer();
  // Contador persistido: sobrevive a reinicios y nunca repite.
